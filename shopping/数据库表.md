...
| username   | VARCHAR  | 20   | ❌      | -                     | 用户名，唯一，不能使用@符号 |
...
| 字段名                                                       | 类型     | 长度 | 允许空 | 默认值            | 备注                            |
| :----------------------------------------------------------- | :------- | :--- | :----- | :---------------- | :------------------------------ |
| xxxxxxxxxx5 1{2  "code": 1,3  "msg": "success",4  "data": null5}json | INT      | -    | ❌      | AUTO_INCREMENT    | 主键，购物车记录ID              |
| user_id                                                      | INT      | -    | ❌      | -                 | 用户ID，关联users.user_id       |
...
| address_id   | INT      | -    | ✅      | NULL              | 收货地址ID，关联user_addresses.address_id（可选）     |
